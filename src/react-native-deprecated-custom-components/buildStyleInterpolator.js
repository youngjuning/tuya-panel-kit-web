...
const inline = function inline(fnStr, replaceWithArgs) {
  const parameterNames =
    fnStr.slice(fnStr.indexOf('(') + 1, fnStr.indexOf(')')).match(ARGUMENT_NAMES_RE) || [];
  const replaceRegexStr = parameterNames
  ...
  const newFnBody = fnBody.replace(replaceRegex, function(parameterName) {
    const indexInParameterNames = parameterNames.indexOf(parameterName);
    const replacementName = replaceWithArgs[indexInParameterNames];
...
const setNextValAndDetectChange = function setNextValAndDetectChange(name, tmpVarName) {
  return (
...
const computeNextValLinear = function computeNextValLinear(anim, from, to, tmpVarName) {
  ...
  const roundOpen = hasRoundRatio ? `Math.round(${roundRatio} * ` : '';
  const roundClose = hasRoundRatio ? `) / ${roundRatio}` : '';
  fn += `  ${tmpVarName} = ${roundOpen}(${from} * (1 - ratio) + ${to} * ratio)${roundClose};\n`;
  return fn;
};
const computeNextValLinearScalar = function computeNextValLinearScalar(anim) {
  return computeNextValLinear(anim, anim.from, anim.to, 'nextScalarVal');
};
const computeNextValConstant = function computeNextValConstant(anim) {
  const constantExpression = JSON.stringify(anim.value);
  return `  nextScalarVal = ${constantExpression};\n`;
};
const computeNextValStep = function computeNextValStep(anim) {
  return `  nextScalarVal = value >= ${anim.threshold} ? ${anim.to} : ${anim.from};\n`;
};
const computeNextValIdentity = function computeNextValIdentity(anim) {
  return '  nextScalarVal = value;\n';
};
const operationVar = function operationVar(name) {
  return `${name}ReuseOp`;
};
const createReusableOperationVars = function createReusableOperationVars(anims) {
  let ret = '';
...
const newlines = function newlines(statements) {
  return `\n${statements.join('\n')}\n`;
};
const computeNextMatrixOperationField = function computeNextMatrixOperationField(
  anim,
  name,
  dimension,
  index
) {
  const fieldAccess = `${operationVar(name)}[${index}]`;
...
const setNextMatrixAndDetectChange = function setNextMatrixAndDetectChange(
  orderedMatrixOperations
) {
  const fn = [
  ...
  fn.push(...inline(MatrixOps.unroll, ['transformMatrix'].concat(unrolledVars)));
  for (let i = 0; i < orderedMatrixOperations.length; i++) {
    const opName = orderedMatrixOperations[i];
    if (i === 0) {
      fn.push(...inline(MatrixOpsInitial[opName], ['transformMatrix', operationVar(opName)]));
    } else {
      fn.push(...inline(MatrixOps[opName], ['transformMatrix', operationVar(opName)]));
    }
  }
  fn.push(
...
const createFunctionString = function createFunctionString(anims) {
  const orderedMatrixOperations = [];
...
const buildStyleInterpolator = function buildStyleInterpolator(anims) {
